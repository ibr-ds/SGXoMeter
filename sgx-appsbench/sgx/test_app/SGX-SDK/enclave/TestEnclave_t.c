...
static sgx_status_t SGX_CDECL sgx_ecall_run_bench(void* pms)
{
	...
	ecall_run_bench(ms->ms_test_id, ms->ms_thread_id);


	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_run_bench_with_transitions(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_run_bench_with_transitions_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_run_bench_with_transitions_t* ms = SGX_CAST(ms_ecall_run_bench_with_transitions_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ecall_run_bench_with_transitions(ms->ms_test_id, ms->ms_thread_id);
...
SGX_EXTERNC const struct {
	...
	8,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_start_bench, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_pause_bench, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_stop_bench, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_cleanup_bench, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_run_bench, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_run_bench_with_transitions, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_set_config, 0, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[11][8];
} g_dyn_entry_table = {
	11,
	{
		{0, 0, 0, 0, 0, 0, 0, 0, },
